...
// ── MODEL MAP ────────────────────────────────────────────
// Accepts both short names from the frontend AND full OpenRouter slugs.
// Add any alias your frontend might send here.
const MODEL_MAP = {
// Haiku aliases
...
// Legacy llama IDs the old frontend sent — remap to Haiku so they still work
“meta-llama/llama-3.3-70b-instruct”: “anthropic/claude-haiku-4-5”,
};
...
// Already a valid OpenRouter “provider/model” slug → pass through
...
console.log(`AI request | raw: "${model}" → resolved: "${resolvedModel}" | ${prompt.length} chars`);
...
if (!text) {
  ...
  return res.status(502).json({ error: "Empty response from OpenRouter", detail: data });
}
...
// PING — frontend polls this to show connection status
app.get(”/api/ping/:token”, (req, res) => {
const s = getSession(req.params.token);
res.json({ connected: s.connected });
});

// POLL — Roblox plugin calls this to get queued commands
...
// HEARTBEAT — alternative keep-alive from plugin
...
// COMMAND — frontend queues a script to send to Studio
...
// RESULT POST — plugin posts execution result back
...
// RESULT GET — frontend polls for execution result
...
// DISCONNECT — plugin signals it’s closing
...
// Test — fires a real minimal OpenRouter call to verify the key works
...
// ── SESSION CLEANUP ───────────────────────────────────────
// Mark sessions as disconnected after 5 min of no heartbeat
setInterval(() => {
